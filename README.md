# Priyanka-Maur
Hi 👋, I'm Priyanka Maurya

📊 Aspiring Data Analyst | Turning Data into Insights

---

👩‍💻 About Me

- 🎓 B.Tech Student
- 📈 Passionate about Data Analysis & Data Visualization
- 💡 Skilled in Advanced Excel, Python, SQL, and Power BI
- 🔍 Love transforming raw data into meaningful insights
- 🚀 Currently working on real-world data analytics projects

---

🛠️ Tech Stack

Excel 
Python
Excel
SQL
Power BI
---

📊 Data Analytics Skills

- Data Cleaning & Transformation
- Data Visualization
- Dashboard Development
- SQL Querying
- Exploratory Data Analysis (EDA)
- Business Intelligence Reporting

---

🚀 Projects

🛒 Blinkit Sales Dashboard

Interactive Power BI dashboard for sales, customer satisfaction, and inventory analysis.

🌐 Portfolio Website

Personal portfolio showcasing projects, skills, and achievements.

📈 Data Analysis Projects

Projects using Python, SQL, Excel, and Power BI to derive actionable insights from data.

---



⭐ Always eager to learn, analyze data, and create impactful solutions.
